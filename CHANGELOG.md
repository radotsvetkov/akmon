...
- **Supply-chain advisories.** Updated `bitstream-io` (4.9.0 → 4.10.0, dropping the yanked `core2` dependency in favor of `no_std_io2`), `anyhow` (1.0.102 → 1.0.103, fixing RUSTSEC-2026-0190, an `Error::downcast_mut` unsoundness), and `crossbeam-epoch` (0.9.18 → 0.9.20, fixing RUSTSEC-2026-0204, an invalid pointer dereference in the `Atomic`/`Shared` `Debug`/`Display` impls). No manifest changes; all three resolve within existing version constraints.
...
- **OpenTelemetry GenAI import — be the trust layer on top of *any* agent (decision D-19, Item 9.1):** `akmon otel import <trace.json>` ingests an OTLP/JSON OpenTelemetry GenAI trace (pinned semconv v1.37) into a verifiable AGEF session, so the whole evidence pipeline (`bundle export`/`sign`/`verify`, `agef-verify`) composes on top of any agent that emits OTEL — turning unsigned, non-replayable telemetry into a signed, independently-verifiable evidence bundle. New `akmon-otel` crate; no new third-party dependencies. **Honest partial capture:** message content is opt-in in OTEL and usually absent, so the importer records `capture_level` (`full` = real content present; `structural` = metadata only) into the *signed* session config; `agef-verify` and `akmon bundle verify` surface it (never a bare "verified" for a metadata-only bundle) and accept `--require-capture full`. OTEL-imported sessions are evidence records, not replayable executions — `akmon replay` refuses them.
- **Legacy OpenTelemetry GenAI import (≤ v1.36 message-event form):** `akmon otel import` now also reduces the legacy per-message span-event form (`gen_ai.system`/`user`/`assistant`/`tool.message`, `gen_ai.choice`) that most deployed instrumentations still emit by default — not only the v1.37 structured attributes. Legacy and structured telemetry of the *same* content produce identical content-object hashes (asserted slot-by-slot); genuinely unrecognized `gen_ai.*` events are still refused, so there are no silent drops.
- **`akmon bundle prove-openssl <bundle> --verify-key <hex>` (metric F.1):** emit `statement.bin`, `signature.bin`, and `pubkey.pem` so a third party verifies an Akmon signature with stock `openssl` alone — no Akmon binary, no cloud. Turns the offline-verifiability wedge from a claim into a reproducible fact; the public key is re-encoded to RFC 8410 SPKI PEM with a dependency-free encoder.
- **`akmon bundle keygen --out <file> [--public-out <file>]`:** generate an Ed25519 signing key usable by `akmon bundle sign` (PKCS#8 v2 — the form `openssl genpkey` cannot produce). The private-key file is created `0600` on unix and never overwritten without `--force`; the private key is never printed.
- **Operator-identity binding (AGEF v0.1.3, decision D-20):** bind an accountable operator/human identity to a session — closing the EU AI Act Art. 14 / Art. 12(3) "which operator" gap without PKI, DID, or cloud. `akmon bundle attest <bundle> --key <pkcs8> --operator-id <id> [--display-name --role --org]` records a separately-signed `AGEF-OPERATOR-v1` attestation in an optional `manifest.operator_attestations[]` (the `AGEF-SIG-v1` head signature and `prove-openssl` bytes stay byte-untouched). `akmon bundle verify` and `agef-verify` gain `--operator-key` / `--require-operator` / `--require-operator-key`; `akmon bundle prove-openssl --operator-key` makes the operator claim verifiable with stock `openssl` too. Verification attaches trust to the **key**, never to the self-asserted identity string (out-of-band key trust); absent ⇒ "unattributed" and still valid.
- **End-to-end proof harness (Item 9.2):** a single integration test drives a real-framework OTEL trace (a provenance-documented `opentelemetry-instrumentation-openai-v2` legacy fixture) through `import → sign → verify → prove-openssl → openssl`, asserting honest `structural` capture (`--require-capture full` fails) and the offline openssl verification — with a committed walkthrough tutorial pointing at the same fixture and test.
...
- **AGEF spec version bumped to `0.1.3`** (from `0.1.2`) — an additive change for the optional `manifest.operator_attestations[]` field (decision D-20). Compatibility is unchanged: `0.1.1`/`0.1.2` readers still accept `0.1.3` bundles and ignore the field; unattributed bundles serialize byte-identically to `0.1.2`.
