...
Phoenix moves through **router, controller/live, context, schema** in that order. A plan file forces the agent to state that order before editing, which reduces half-written plugs or misnamed `assigns`.
